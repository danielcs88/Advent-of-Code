...
# %%
df = valid_passports(part1[:-1])[0]

# %%
df

# %%
df["eyr"] = df["eyr"].astype("int32")

# %%
df["byr"] = df["byr"].astype("int32")

# %%
df["iyr"] = df["iyr"].astype("int32")

# %%
height_measurements = (df["hgt"].str.endswith("cm")) | (df["hgt"].str.endswith("in"))

# %%
df = df[height_measurements]
...
def height_conversion(x):
    if not x.endswith("in"):
        return x
    height = x[:-2]
    return f"{float(height) * 2.54}cm"


# %%
height_conversion("170cm")

# %%
df["hgt"] = df["hgt"].apply(height_conversion)

# %%
df["hgt"] = [float(x[:-2]) for x in df["hgt"]]

# %%
height_criteria = (df["hgt"] >= 150) & (df["hgt"] <= 193)

# %%
df = df[height_criteria]
...
# %%
df = df[year_criterion]

# %%
passport_criteria = [len(x) == 9 for x in df["pid"]]

# %%
df = df[passport_criteria]
...
def HexCode(str):
    ...
    # matched the ReGex
    return bool(re.search(p, str))


# %%
hex_criteria = [HexCode(x) for x in df["hcl"]]
