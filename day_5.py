...
# %%
def seat_id(boarding_pass, summary=False):
    """
    Parses through boarding pass string.
    """

    row = "".join("0" if i == "F" else "1" for i in boarding_pass[:7])
    col = "".join("0" if i == "L" else "1" for i in boarding_pass[-3:])
    row = int(row, 2)
    col = int(col, 2)
...
# %%
plus_one = [x for x in tickets if x + 1 in tickets]

# %%
minus_one = [x for x in tickets if x - 1 in tickets]

# %%
seats = sorted(set(plus_one + minus_one))

# %%
possibilities = list(range(np.min(seats), np.max(seats) + 1))

# %%
seat = set(possibilities) ^ set(seats)
print(seat)
