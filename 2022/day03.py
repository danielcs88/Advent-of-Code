...
# %%
def sum_priorities(ransack: list[str]) -> int:
    # sourcery skip: remove-redundant-constructor-in-dict-union
    priority = dict(
        zip([chr(i) for i in range(ord("a"), ord("z") + 1)], range(1, 26 + 1))
    ) | dict(
        zip([chr(i) for i in range(ord("A"), ord("Z") + 1)], range(27, 27 + 26 + 1))
    )
    ...
    for i in ransack:
        ...
        left = i[:middle]
        right = i[middle:]
        shared = list(set(left) & set(right))[0]
        count += priority[shared]
    print(count)
    return count


# %%
input_03 = open("input_03.txt", encoding="utf-8").read().splitlines()
# from aocd import get_data from startup import main input_03 = get_data(day=3,
# year=2022)
...
# %%
def sum_priorities_part_2(ransack: list[str]) -> int:
    priority = dict(
        zip([chr(i) for i in range(ord("a"), ord("z") + 1)], range(1, 26 + 1))
    ) | dict(
        zip([chr(i) for i in range(ord("A"), ord("Z") + 1)], range(27, 27 + 26 + 1))
    ...
    count = 0
    for i in chunker(ransack, 3):
        one, two, three = i
        shared = list(set(one) & set(two) & set(three))[0]
        # print(shared)
        count += priority[shared]
    print(count)
    return count
...
# %%
sum_priorities_part_2(test)

# %%
sum_priorities_part_2(input_03)
