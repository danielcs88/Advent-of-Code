...
# strategy guide?**
#
# To begin, [get your puzzle input](input_02.txt).
...
def splitting_rationale(text: str) -> list[str]:
    r"""
    'A Y\nB X\nC Z' -> ['A Y', 'B X', 'C 'Z]
    """
    return text.split("\n")
...
# %%
splitting_rationale(test)


# %%
input_02 = open("input_02.txt", encoding="utf-8").read()
# from startup import main

# input_02 = get_data(day=2, year=2022)

# %%
def rock_paper_scissors_game(game: str) -> int:
    r"""['A Y', 'B X', 'C Z']"""
    ...
    opponent_shape = translation_rps[game[0]]
    your_shape = translation_rps[game[1]]
    ...
    # 1. Determine outcome -> score
    # 2. Determine shape -> score
    # 3. Sum
    return sum(
        (
            outcome_score[mechanics[(opponent_shape, your_shape)]],
...
# %%
def score(x):
    return sum(rock_paper_scissors_game(game.split()) for game in splitting_rationale(x))

# %%time
print(score(test))
# %%
print(score(input_02))
...
# %%
def reverse_non_unique_dict(
    non_unique_dict: dict[tuple[str], str]
) -> dict[str, dict[str]]:
    r"""{ ("r", "r"): "d", ("p", "p"): "d", ("s", "s"): "d"
    } -> {
        "d":{
    ...
    values = [dict(v) for v in obj.values()]
    return dict(zip(obj.keys(), values))
    # return dict(zip(obj.keys(), dict(obj.values())))


# %%
def rock_paper_scissors_game_2(game: list[str]) -> int:
    r"""['A', 'Y']
    ['B', 'X'] ['C', 'Z']"""
    ...
    strategy_secret = {"X": "Loss", "Y": "Draw", "Z": "Win"}
    shape_score = {"Rock": 1, "Paper": 2, "Scissors": 3}
    outcome_score = {"Win": 6, "Draw": 3, "Loss": 0}

    # opponent_shape = translation_rps[game[0]] your_shape =
    # translation_rps[game[1]]
    ...
    opponent_shape = translation_rps[game[0]]
    strategy = strategy_secret[game[1]]

    strategy_mechanics = reverse_non_unique_dict(mechanics)
    your_shape = strategy_mechanics[strategy][opponent_shape]

    # 1. Determine outcome -> score
    # 2. Determine shape -> score
    # 3. Sum
    return sum((outcome_score[strategy], shape_score[your_shape]))


# %%
print([rock_paper_scissors_game_2(_.split()) for _ in splitting_rationale(test)])

# %%
def score_2(x):
    return sum(rock_paper_scissors_game_2(game.split()) for game in splitting_rationale(x))

# %%
score_2(test)
# %%
print(score_2(input_02))
