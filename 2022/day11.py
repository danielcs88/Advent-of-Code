...
# until each monkey has had one turn. The process of each monkey taking a single
...
# %%
test_11 = open_input("test_11.txt")
input_11 = open_input("input_11.txt")


# %%
def monkey_dict(monkey_instructions: str):
    monkeys = []
    for monkey in monkey_instructions.split("\n\n"):
        monkey = monkey.splitlines()[1:]
        starting_items, operation, test, if_true, if_false = monkey
        # print(test.split("Test: divisible by ")[1])
        monkeys.append(
            dict(
                items=[int(x) for x in starting_items.split(": ")[1].split(", ")],
                operation=[
                    lambda old: eval(o) for o in operation.split("Operation: new = ")
                ][1],
                test=int(list(test.split("Test: divisible by "))[1]),
                throw={
                    True: [
                        int(x) for x in if_true.split("If true: throw to monkey ")[1]
                    ][0],
                    False: [
                        int(x) for x in if_false.split("If false: throw to monkey ")[1]
                    ][0],
                },
            )
        )
...
# %%
def productive_monkey_in_the_middle(
    instructions: str, rounds: int = 20, relief=True
) -> int:
    monkey_business = monkey_dict(instructions)
    active = defaultdict(int)
    relief_divisor = 3 if relief else prod(m["test"] for m in monkey_business)

    for _ in range(rounds):
        for i, monkey in enumerate(monkey_business):
            for item in monkey["items"]:
                active[i] += 1
                worry_level = (
                    monkey["operation"](item) // relief_divisor
                    if relief
                    else monkey["operation"](item) % relief_divisor
                )
                test_result = worry_level % monkey["test"] == 0
                destination = monkey["throw"][test_result]
                monkey_business[destination]["items"].append(worry_level)
            monkey["items"] = []
    return prod(sorted(active.values(), reverse=True)[:2])


# %%
print(productive_monkey_in_the_middle(test_11))

# %%
print(productive_monkey_in_the_middle(input_11))
...
# again from the initial state in your puzzle input, **what is the level of
# monkey business after 10000 rounds?**

# %%
print(productive_monkey_in_the_middle(input_11, relief=False, rounds=int(10e3)))
