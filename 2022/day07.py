...
# %%
test = """$ cd /
$ ls dir a 14848514 b.txt 8504156 c.dat dir d $ cd a $ ls dir e 29116 f 2557 g
62596 h.lst $ cd e $ ls 584 i $ cd .. $ cd .. $ cd d $ ls 4060174 j 8033020
d.log 5626152 d.ext 7214296 k"""
...
# %%
def part_1(history: str, solution=True) -> int | defaultdict:
    cwd = Path("/")
        ...
        match line.split():
            ...
            case [size, _] if size.isdigit():
                ...
                for path in [cwd, *cwd.parents]:
                    dirs[path] += size
    if solution:
        return sum(x for x in dirs.values() if x <= 1e5)
    else:
        return dirs
...
input_07 = open_input("input_07.txt")
# from aocd import get_data from startup import main

# input_07 = get_data(day=7, year=2022)

# %%
print(part_1(input_07))
...
# **unused** space must currently be `21618835`, which isn't quite the
...
# these, choose the **smallest**: `d`, increasing unused space by
# **`24933642`**.
#
...
# %%
def part_2(history: str, solution=True) -> int | defaultdict:
    cwd = Path("/")
    dirs = defaultdict(int)
        ...
        match line.split():
            ...
            case [size, _] if size.isdigit():
                ...
                for path in [cwd, *cwd.parents]:
                    dirs[path] += size
    if solution:
        return min(x for x in dirs.values() if dirs[Path("/")] - x <= 70e6 - 30e6)
    else:
        return dirs
