...
def cycles(instructions, padding=True):
    qty = 1

    cycle_vals = [0] if padding else []
    for line in instructions.splitlines():
        if line != "noop":
            _, add_x = line.split()
            cycle_vals.extend((qty, qty))
            qty += int(add_x)
        else:
            cycle_vals.append(qty)
    # cycle_vals.append(qty)
    return cycle_vals


# %%
test_2 = open_input("test_10.txt")

# %%
def signal_strenghts(cycle_list: str) -> int:
    return sum(cycles(cycle_list)[cycle] * cycle for cycle in range(20, 260, 40))


# %%

input_10 = open_input("input_10.txt")

# %%
print(signal_strenghts(input_10))
...
# is no such thing as "vertical position": if the sprite's horizontal position
...
# produces a **lit** pixel (`#`); otherwise, the screen leaves the pixel
# **dark** (`.`).
#
...
def part_two(instructions: str):
    ...
    registers = cycles(instructions, padding=False)

    for i in range(0, len(registers), 40):
        for j in range(40):
            # print(f'i+j: {i+j}   registers[i + j]: {registers[i +
            # j]}\tj:{j}\t#:{abs(registers[i + j]-j)<=1}')
            print("#" if abs(registers[i + j] - j) <= 1 else " ", end="")
        print()
