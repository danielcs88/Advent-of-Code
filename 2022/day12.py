...
#          - Left: `(0, -1)`
#          - Right: `(0, 1)`
# 2. Keep track of count and visited squares
...
def p1(f):
    ...
    while len(bfs) > 0:
        ...
        for q in adj(*p):
            # print("q", q) print(ord(grid.get(q, "~")))
...
def p2(f):
    grid: dict[tuple[int, int], str] = {
        (i, j): "a" if x == "S" else x
        for i, row in enumerate(f.splitlines())
