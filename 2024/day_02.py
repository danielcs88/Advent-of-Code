...
# %%
def is_valid_sequence(sequence: pd.Series) -> bool:
    """
    Check if a sequence is monotonic and has differences between 1 and 3.

    Args:
        sequence: pandas Series of numbers to check

    Returns:
        bool: True if sequence meets both monotonic and difference criteria
    """
    return (
        sequence.is_monotonic_increasing | sequence.is_monotonic_decreasing
    ) & sequence.diff().abs().dropna().between(1, 3).all()


# %%
@aoc_answer_display
def safe_row_count(input_str: str) -> int:
    reports = [
        pd.Series(int(num) for num in row.split()) for row in input_str.splitlines()
    ]
    return sum(report.pipe(is_valid_sequence) for report in reports)


# %%
safe_row_count(input_02)
...
# %%
@aoc_answer_display
def safe_row_count_w_problem_dampener(input_str: str) -> int:
    ...
    for row in reports:
        ...
        for i in range(report_size):
            attempt = row.drop(i)
            if attempt.pipe(is_valid_sequence):
                safe_count += 1
                break
    return safe_count
...
# %%
safe_row_count_w_problem_dampener(input_02)
