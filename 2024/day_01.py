...
def text_to_array(input_str: str, sort: bool = True) -> tuple[list[int]]:
    columns = (map(int, row.split()) for row in input_str.splitlines())
    
    return map(sorted, zip(*columns)) if sort else zip(*columns)


def total_distance_between_lists(left_list: list[int], right_list: list[int]) -> int:
    return sum(abs(x - y) for x, y in zip(left_list, right_list))


total_distance_between_lists(*text_to_array(example))

# %%
input_01 = aoc_open_input("input_01.txt")

# %%
aoc_answer_display(total_distance_between_lists(*text_to_array(input_01)))
...
def similarity_score(input_str:str) -> int:
    left, right = text_to_array(input_str, False)
    return sum(num * Counter(right)[num] for num in left)


# %%
aoc_answer_display(similarity_score(input_01))
