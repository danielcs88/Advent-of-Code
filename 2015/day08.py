...
test_08 = aoc_open_input("test_08.txt")


# %%
def part_one(input_str: str) -> int:
    count_chars_string_code = 0
    count_mem_chars = 0

    for i in input_str.splitlines():
        count_chars_string_code += len(i)
        count_mem_chars += len(eval(i))

    result = count_chars_string_code - count_mem_chars
...
# of code, you should now **encode each code representation as a new string**
# and find the number of characters of the new encoded representation, including
...
def part_two(input_str: str) -> int:
    ...
    for i in input_str.splitlines():
        code_repr = '"' + i.replace("\\", "\\\\").replace('"', '\\"') + '"'
        count_code_repr += len(code_repr)
        count_chars_string_code += len(i)

    result = count_code_repr - count_chars_string_code
    pyperclip.copy(result)
    print(result)
    return result
