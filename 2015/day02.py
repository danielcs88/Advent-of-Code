...
input_02 = aoc_open_input("input_02.txt").splitlines()
...
def part_one(input_str: str) -> int:
    ...
    for line in input_str:
        ...
        result_dict = dict(zip(["l", "w", "h"], [l, w, h]))
        result_dict["initial_requirement"] = sum([2 * l * w, 2 * w * h, 2 * h * l])
        result_dict["smaller_side"] = prod(sorted([l, w, h])[:2])
        result_dict["final_result"] = (
            result_dict["initial_requirement"] + result_dict["smaller_side"]
        )
        final_result += result_dict["initial_requirement"] + result_dict["smaller_side"]
...
def part_two(input_str: str) -> int:
    ...
    for line in input_str:
        ...
        l, w, h = dimensions
        result_dict = dict(zip(["l", "w", "h"], [l, w, h]))
        result_dict["ribbon"] = sum(sorted([l, w, h])[:2] * 2)
        result_dict["bow"] = prod(dimensions)
        result_dict["bow + ribbon"] = result_dict["ribbon"] + result_dict["bow"]
        bow_n_ribbon += result_dict["bow + ribbon"]
        results.append(result_dict)
