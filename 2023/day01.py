...
# %%
def line_num(line: str) -> int:
    nums = [char for char in line if char.isdigit()]
    return int(nums[0] + nums[-1])


# %%
def solution(input_string: str, verbose: bool = False) -> int:
    result = (line_num(line) for line in input_string.splitlines())
    return list(result) if verbose else sum(result)


# %%
input_str = aoc_open_input("input_01.txt")
...
# %%
def solution_2(input_string: str) -> int:
    ...
    return sum(int(digits.get(n[0], n[0]) + digits.get(n[-1], n[-1])) for n in numbers)
